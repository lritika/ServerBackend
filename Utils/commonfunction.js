/**
 * Created by cl-macmini-63 on 1/13/17.
 */
...
AWS.config.update({accessKeyId : s3config.accessKeyId, secretAccessKey : s3config.secretAccessKey,signatureVersion: 'v4'});
...
let uploadFile = function(tempPath,fileName,type,callback)
{
    async.parallel([
            ...
            fs.readFile(tempPath,function(err,file){
                ...
                s3.putObject(params, function (err, data) {
                    if (err) {
                        ...
                        console.log("success in original");
                        cb();
                    }
                });
            });
        ...
        function(cb)
        {
            ...
            gm(tempPath).quality(100).resize('250','250','^').gravity('Center').crop('250','250').autoOrient().write(thumbFile, function (err) {

                if(err) {

                    console.log("error in thumb", JSON.stringify(err));
                    cb(err);
                }
                else {
...
var generateRandomStringBigger = function () {
    return randomstring.generate(16);
};
function MergeDataWithKey(dData,vData,objKey){
    var outputData = [];
    dData.forEach(function(val,index){
        var localDestinationData = val;
        localDestinationData[objKey] = vData[index];
        outputData.push(localDestinationData);
    });
    return outputData;
...
module.exports = {
    MergeDataWithKey:MergeDataWithKey,
    generateRandomStringBigger:generateRandomStringBigger,
